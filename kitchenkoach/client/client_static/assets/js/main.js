(function ($) {
  ...
  jQuery(document).ready(function () {
    ...
    let getEditAddress = document.querySelectorAll(".edit-address");

    let clickEvent = () => {
      frontAddress.style.display = "none";
      editAddressForm.style.display = "block";
    };

    getEditAddress.forEach((item) => {
      item.addEventListener("click", clickEvent);
    });

    if (backBtn) {
      backBtn.addEventListener("click", function () {
        frontAddress.style.display = "block";
    ...
    $(".ice-slider-for").slick({
      ...
      arrows: true,
      prevArrow:
        '<i class="fas fa-long-arrow-alt-left slider-nav nav-left"></i>',
      nextArrow:
        '<i class="fas fa-long-arrow-alt-right slider-nav nav-right"></i>',
      responsive: [
        {
          breakpoint: 576,
          settings: {
